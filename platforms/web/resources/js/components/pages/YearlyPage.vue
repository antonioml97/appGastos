                ...
                <div class="flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
                    <div class="min-w-0">
                        ...
                        <p class="mt-4 max-w-2xl text-base leading-8 text-white/72">
                            Vista global del ano con ingresos, gastos y balance acumulado para entender como evoluciona el ano completo.
                        </p>
                    </div>

                    <div class="w-full max-w-sm rounded-[1.5rem] border border-white/10 bg-[var(--color-ink-soft)]/45 p-4">
                        <label class="mb-3 block text-sm font-semibold text-white/78">Cambiar ano</label>
        ...
        <aside class="space-y-6 self-start">
            <section class="rounded-[2rem] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.09),rgba(255,255,255,0.03))] p-6 backdrop-blur-lg sm:p-8">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <p class="text-sm uppercase tracking-[0.2em] text-[var(--color-mint)]">Categorias</p>
                    ...
                    <span class="rounded-full border border-white/10 bg-white/8 px-4 py-2 text-sm text-white/70">
                        {{ state.categoryBreakdown.length }} categorias
                    </span>
                </div>

                <div class="mt-7 grid gap-4 sm:grid-cols-2">
                    <article class="rounded-[1.35rem] border border-white/10 bg-[var(--color-ink-soft)]/60 p-4">
                        <p class="text-xs uppercase tracking-[0.24em] text-white/50">Top categoria</p>
                        ...
                        <p class="mt-2 text-sm text-white/60">
                            {{ topCategory ? formatCurrency(topCategory.total) : 'Aun no hay gastos' }}
                        </p>
                    </article>
                    <article class="rounded-[1.35rem] border border-white/10 bg-[var(--color-ink-soft)]/60 p-4">
                        <p class="text-xs uppercase tracking-[0.24em] text-white/50">Reparto activo</p>
                        <p class="mt-3 text-xl font-bold text-white">{{ activeCategoryShare }}</p>
                        <p class="mt-2 text-sm text-white/60">Concentracion de la categoria principal</p>
                    </article>
                </div>
                ...
                <div class="mt-8 space-y-4">
                    ...
                    <div v-if="rankedCategories.length === 0" class="rounded-[1.5rem] border border-dashed border-white/12 bg-white/4 p-6 text-sm leading-7 text-white/65">
                        Todavia no hay gastos en este ano. Cuando registres movimientos apareceran aqui el reparto por categorias.
                    </div>
                </div>
            </section>
        </aside>
...
const activeCategoryShare = computed(() => topCategory.value ? `${topCategory.value.share.toFixed(1)}%` : '0,0%');
